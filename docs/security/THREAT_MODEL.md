...
| :------------------ | :---------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------- | :--------------- |
| **Spoofing**        | Attacker impersonates a valid user using stolen credentials.      | **Firebase Auth** requires MFA (recommended) and short-lived tokens. **HSTS** prevents protocol downgrade attacks.                 | ✅ Mitigated     |
| **Tampering**       | Attacker modifies API requests (e.g., changes `role=admin`).      | **Pydantic Validation** enforces strict schemas. **Firestore Rules** validated server-side. **CSP** prevents XSS/Injection.        | ✅ Mitigated     |
...
| **Info Disclosure** | Sensitive data (PII) leaked in logs or error messages.            | **Pydantic** strips sensitive fields. **Cloud Run** env vars are secret-managed. **Permissions-Policy** restricts hardware access. | ✅ Mitigated     |
...
| **Supply Chain**    | Vulnerable dependencies or container base images.                 | **SCA** (pip-audit, npm audit) and **Container Scan** (Trivy) in CI/CD pipeline.                                                   | ✅ Mitigated     |
